...
if select_parm in ['Temperature', 'RH']:
    ...
    temp_mask = ((data['value'] <= legend_bound[0]) | (data['value'] >= legend_bound[1]))

    if temp_mask.sum() != 0:
        coord = data[temp_mask]['geometry'].to_list()
        coord_temp = data[temp_mask]['value'].to_list()
